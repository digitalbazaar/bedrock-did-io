...
bedrock.events.on('bedrock.init', () => {
  ...
  didIo._cache = new LruCache(_coerceCacheConfig(cfg.cache));
});
...
export function _coerceCacheConfig(cacheConfig) {
  const result = {...cacheConfig};
  // coerce `maxSize` w/o `sizeCalculation` to `max`
  if(result.maxSize !== undefined && result.sizeCalculation === undefined) {
    result.max = result.maxSize;
    delete result.maxSize;
  }
  // coerce `maxAge` to `ttl`
  if(result.maxAge !== undefined) {
    result.ttl = result.maxAge;
