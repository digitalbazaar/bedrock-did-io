...
export function _coerceCacheConfig(cfg) {
  // coerce `maxSize` w/o `sizeCalculation` to `max`
  if(cfg.maxSize !== undefined && cfg.sizeCalculation === undefined) {
    cfg.cache = {...cfg.cache, max: cfg.maxSize};
    delete cfg.maxSize;
  }
  // coerce `maxAge` to `ttl`
  if(cfg.maxAge !== undefined) {
    cfg.cache = {...cfg.cache, ttl: cfg.maxAge};
    delete cfg.maxAge;
  }
}
